...
export function ContainerModal({
  ...
  const [repositories, setRepositories] = useState<
    { id: number; name: string; clone_url: string; private: boolean }[]
  >([]);
              ...
              {[1, 2, 3, 4].map((s) => (
                <div
                  ...
                  className={`h-1 flex-1 rounded-full transition-colors ${
                    isEditMode || step >= s ? "bg-blue-500" : "bg-white/10"
                  } ${s !== 4 ? "mr-2" : ""}`}
                />
              ))}
            </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <Github className="absolute top-1/2 left-3 h-4 w-4 -translate-y-1/2 text-neutral-500" />
                      <input
                        id="github-username"
                        type="text"
                    ...
                    <div className="max-h-40 overflow-y-auto rounded-lg border border-white/10 bg-black/40 p-1">
                      {repositories.map((repo) => (
                        ...
                        >
                          ...
                          {repo.private && (
                            <span className="ml-2 rounded bg-neutral-800 px-1 text-[10px] text-neutral-500 uppercase">
                              Private
                            </span>
                          )}
                        </button>
