...
export class ActionLogModal extends BaseComponent {
    ...
    async expectLoaded() {
        ...
        // Wait for the title
        await expect(
            this.page.getByRole("heading", { name: /activity log/i })
        ).toBeVisible({ timeout: 10_000 });

        // Wait for table or no logs message
        const hasTable = await this.table().isVisible().catch(() => false);
        const hasNoLogs = await this.noLogsMessage().isVisible().catch(() => false);
    ...
    async expectSortedBy(
        column: "action" | "collaborator" | "timestamp",
        direction: "asc" | "desc"
    ) {
    ...
    async getLogEntries(): Promise<Array<{ action: string; collaborator: string; timestamp: string }>> {
