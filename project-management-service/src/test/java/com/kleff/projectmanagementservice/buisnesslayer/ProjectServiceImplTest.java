...
class ProjectServiceImplTest {
        ...
        void deleteProject_WithValidId_MarksProjectAsDeleted() {
            ...
            assertThat(result).isNotNull();
            // Note: findByProjectId is called multiple times internally, so we don't verify exact count
        }
...
//            verify(projectRepository).save(any(Project.class)); // Soft delete via save
        ...
        void deleteProject_WithExistingProject_PerformsFullCleanup() {
            ...
            // Mock collaborator repository to return collaborator objects when findById is called
            ...
            verify(collaboratorService, times(collaborators.size()))
                    .removeCollaborator(Mockito.anyString(), Mockito.anyString());
        ...
        void deleteProject_WhenCollaboratorRemovalFails_RollsBackTransaction() {
            ...
            // Mock collaborator repository to return collaborator objects when findById is called (lenient)
            for (Collaborator collaborator : collaborators) {
            ...
            doThrow(new RuntimeException("Collaborator service error"))
                    .when(collaboratorService).removeCollaborator(Mockito.anyString(), Mockito.anyString());
